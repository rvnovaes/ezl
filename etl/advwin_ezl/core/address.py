        ...
        for row in rows:
            ...
            try:
                ...
                address_type, created = AddressType.objects.get_or_create(
                    name=address_type_name, create_user=user)
                for person in persons:
                    country = Country.objects.filter(
                        name__unaccent__iexact=row['country'].strip(
                        ) if row['country'] else '') or Country.objects.filter(
                            name__unaccent__iexact='Brasil')
                    state = State.objects.filter(
                        initials__iexact=row['state'].strip(
                        ) if row['state'] else '') or State.objects.filter(
                            name__iexact='ESTADO-INVÁLIDO')
                    city = City.objects.filter(
                        name__unaccent__iexact=row['city'].
                        strip() if row['city'] else '') or City.objects.filter(
                    ...
                    self.debug_logger.debug(
                        ...
                        % (str(person.id), str(row['street'] or '', ),
                           str(row['number'] or ''),
                           str(row['complement'] or ''),
                           str(row['city_region'] or ''),
                           str(row['zip_code'] or ''), str(address_type.id),
                           str(state.id), str(city.first().id),
                           str(country.first().id), str(user.id), str(user.id),
                           self.timestr))
