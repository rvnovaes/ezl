...
class TaskFilter(df.FilterSet):
    ...
    reminder_init = df.DateFilter(widget = SelectDateWidget(years=range(2008, year+1)),name='Lembrete',lookup_expr='rem',label="Lembrete")
    reminder_end = df.DateFilter(widget = SelectDateWidget(years=range(2008, year+1)),name='Lembrete',lookup_expr='rem',label="a")
    deadline_init = df.DateFilter(widget = SelectDateWidget(years=range(2008, year+1)),name='PrazoFinal',lookup_expr='dl',label="Prazo Final")
    deadline_end = df.DateFilter(widget = SelectDateWidget(years=range(2008, year+1)),name='PrazoFinal',lookup_expr='dl',label="a")
