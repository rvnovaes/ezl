...
from core.models import Person, Invite, Office, CustomSettings, EmailTemplate
...
def custom_settings_post_save(sender, instance, created, **kwargs):
    if created:
        ...
        instance.default_user.groups.add(
            Group.objects.filter('Correspondente-{}'.format(custtom_settings.office.pk)).first())
        status_to_show = [
            TaskShowStatus(custtom_settings_id=instance.id, create_user=instance.create_user, status_to_show=TaskStatus.OPEN, send_mail_template=EmailTemplate.objects.filter(
                template_id='d-a9f3606fb333406c9b907fee244e30a4').first()), 
            TaskShowStatus(custtom_settings_id=instance.id, create_user=instance.create_user, status_to_show=TaskStatus.RETURN), 
            TaskShowStatus(custtom_settings_id=instance.id, create_user=instance.create_user, status_to_show=TaskStatus.REFUSED),
            TaskShowStatus(custtom_settings_id=instance.id, create_user=instance.create_user, status_to_show=TaskStatus.ACCEPTED, send_mail_template=EmailTemplate.objects.filter(
                template_id='d-ae35cc53722345eaa4a4adf521c3bd81').first()),                      
            TaskShowStatus(custtom_settings_id=instance.id, create_user=instance.create_user, status_to_show=TaskStatus.FINISHED, send_mail_template=EmailTemplate.objects.filter(
        ...
        task_workflows = [
            TaskWorkflow(custtom_settings_id=instance.id, create_user=instance.create_user, 
                task_from=TaskStatus.REQUESTED, task_to=TaskStatus.ACCEPTED_SERVICE, responsible_user=instance.default_user),    
            TaskWorkflow(custtom_settings_id=instance.id, create_user=instance.create_user, 
                task_from=TaskStatus.ACCEPTED_SERVICE, task_to=TaskStatus.OPEN, responsible_user=instance.default_user),             
        ]
