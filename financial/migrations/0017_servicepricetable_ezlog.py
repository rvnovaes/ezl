...
def create_service_price_table_ezlog(apps, schema_editor):
	...
	price_filters = ServicePriceTable.objects.filter(office_id=1, client=None, value__gt=0).values('type_task', 'court_district', 'state')
	...
	for price_filter in price_filters:		
		...
		max_value = ServicePriceTable.objects.filter(type_task_id=type_task_id, court_district_id=court_district_id, state_id=state_id, client=None, value__gt=0.0).aggregate(Max('value'))
